  ...
  return (
    <div className="flex justify-evenly items-center justify-center MainDiv">
      <div className=" flex flex-col items-center anouncementsMain">
        <strong className="font-2xl"> Announcements, News & Updates</strong>
        {anouncements.map((anouncement, index) => {
          return (
            <div
              className="w-3/4 p-2 my-2 bg-[#367588] flex items-center text-sm rounded-sm anouncementItems"
              key={index}
            >
              <div>{anouncement.description}</div>
            </div>
          );
        })}
      </div>
